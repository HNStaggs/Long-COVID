...
```{R}
#Extracting everyone's medications from free text data
...
#freqs <- TermDocumentMatrix(rc$medsinfo)
#med_mat <- as.matrix(freqs)
# Calculate the rowSums: term_frequency
#term_frequency <- rowSums(med_mat)
...
#Vet
table(clean %>% filter(participant_type == 'Vet') %>% summarise(vet_s = sex))
prop.table(table(clean %>% filter(participant_type == 'Vet') %>% summarise(vet_s = sex)))

#Civ
table(clean %>% filter(participant_type == 'Civ') %>% summarise(civ_s = sex))
prop.table(table(clean %>% filter(participant_type == 'Civ') %>% summarise(civ_s = sex)))
```
## Gender
```{r}
#All
...
#Vet
table(clean %>% filter(participant_type == 'Vet') %>% summarise(vet_g = gender))
prop.table(table(clean %>% filter(participant_type == 'Vet') %>% summarise(vet_g = gender)))

#Civ
table(clean %>% filter(participant_type == 'Civ') %>% summarise(civ_g = gender))
prop.table(table(clean %>% filter(participant_type == 'Civ') %>% summarise(civ_g = gender)))
```
## Ethnicity
```{r}
#summary(clean)
...
#Vet
table(clean %>% filter(participant_type == 'Vet') %>% summarise(vet_e = ethnicity))
prop.table(table(clean %>% filter(participant_type == 'Vet') %>% summarise(vet_e = ethnicity)))

#Civ
table(clean %>% filter(participant_type == 'Civ') %>% summarise(civ_e = ethnicity))
prop.table(table(clean %>% filter(participant_type == 'Civ') %>% summarise(civ_e = ethnicity)))
```
## Area Type
```{r}
#All
...
#Vet
table(clean %>% filter(participant_type == 'Vet') %>% summarise(vet_a = residence_location_type))
prop.table(table(clean %>% filter(participant_type == 'Vet') %>% summarise(vet_a = residence_location_type)))

#Civ
table(clean %>% filter(participant_type == 'Civ') %>% summarise(civ_a = residence_location_type))
prop.table(table(clean %>% filter(participant_type == 'Civ') %>% summarise(civ_a = residence_location_type)))
```
## Race
```{r}
#All
...
#Vet
table(clean %>% filter(participant_type == 'Vet') %>% summarise(vet_r = race))
prop.table(table(clean %>% filter(participant_type == 'Vet') %>% summarise(vet_r = race)))

#Civ
table(clean %>% filter(participant_type == 'Civ') %>% summarise(civ_r = race))
prop.table(table(clean %>% filter(participant_type == 'Civ') %>% summarise(civ_r = race)))
```
## Degree
```{r}
#All
...
#Vet
table(clean %>% filter(participant_type == 'Vet') %>% summarise(vet_d = edu_degree))
prop.table(table(clean %>% filter(participant_type == 'Vet') %>% summarise(vet_d = edu_degree)))

#Civ
table(clean %>% filter(participant_type == 'Civ') %>% summarise(civ_d = edu_degree))
prop.table(table(clean %>% filter(participant_type == 'Civ') %>% summarise(civ_d = edu_degree)))
```
...
#Vet
table(clean %>% filter(participant_type == 'Vet') %>% summarise(vet_p = pcs_diag))
prop.table(table(clean %>% filter(participant_type == 'Vet') %>% summarise(vet_p = pcs_diag)))

#Civ
table(clean %>% filter(participant_type == 'Civ') %>% summarise(civ_p = pcs_diag))
prop.table(table(clean %>% filter(participant_type == 'Civ') %>% summarise(civ_p = pcs_diag)))
```
## Vaccine Status
```{r}
#All
...
#Vet
table(clean %>% filter(participant_type == 'Vet') %>% summarise(vet_v = vaccboost))
prop.table(table(clean %>% filter(participant_type == 'Vet') %>% summarise(vet_v = vaccboost)))

#Civ
table(clean %>% filter(participant_type == 'Civ') %>% summarise(civ_v = vaccboost))
prop.table(table(clean %>% filter(participant_type == 'Civ') %>% summarise(civ_v = vaccboost)))
```
## Reading Speed
```{r}
#Read: All
...
cptd <- ggplot(clean%>%filter(d<5), aes(d, fill = pcs_diag)) + 
  geom_dotplot(position = 'jitter') +
  theme_minimal() #+
  #scale_x_discrete(limits = c(0:7))
...
#View(pcs_cloud_df)
pcs_cloud_df$color <- if_else(pcs_cloud_df$word == 'Fatigue' | pcs_cloud_df$word == 'Insomnia' |
                                pcs_cloud_df$word == 'BrainFog' | pcs_cloud_df$word == 'Exertion' |
                                pcs_cloud_df$word == 'DaytimeSleepy', 'red', 'grey')

pcs_bar <- ggplot(data = pcs_cloud_df[1:20,], aes(x = reorder(word, +freq), fill = color, y = prop)) + geom_bar(stat = 'identity', show.legend = F) + 
  labs(title = 'Top 20 PASC Symptoms', x = 'Symptom', y = 'Proportion of Sample (n=294)') +
  coord_flip() + 
  theme_minimal() + 
...
clean$vacc_bi[clean$vacc_bi == 0] <- 'None'
...
# Mental Health and Cognitive Symptoms - binary coded if reported
```{r}
...
clean$mental_sum <- (clean$brainfog + clean$anxiety + clean$depression)
```
...
# Update Nan, Nan, Nan subtype to 0, 0, 0 (this is healthy people without PASC)
...
# Run K means
set.seed(2)
kmns_symp <- kmeans(clean[ , c(194:196)], centers = 6)  # Proportion of sleep, mental health, and physical symptoms
...
# Visualize clusters
fviz_cluster(kmns_symp, data = clean[ , 194:196], geom = c('point','text')) + theme_minimal()
...
samp_match <- matchit(participant_type ~ age + sex + edu_degree, clean, 
...
prop.test(x = c(115, 283),
          n = c(303, 806))  # Non-sig difference between samples
```
...
```{r}
all_vars <- c('global_qol_scale', 'infection_frequency', 'initial_symptom_score', 'persistent_symptom_score', 'pre_pandemic',
                'peri_pandemic', 'peri_infection', 'post_infection', 'promis_cog_score', 'hit6_score', 'fatigue_score',
                'gad7_tot_score', 'phq9_total', 'pss_overall_score', 'social_support_score', 'dvprs_score', 'dyspnoea_12_score',
                'BMI', 'duration', 'psqi_glob', 'mental', 'vs_p_rt_mn_s', 'f_c_rt_mn', 'cpt_profile', 'gender', 'race', 'ethnicity', 
              'racial_stress', 'nicotine_use', 'rehab', 'unemployment_covid', 'financial_stress_1', 'func_status_1', 
              'pcfs_grade_status', 'meds', 'tbi_prob', 'tbi_covid', 'vacc_bi', 'cardio_bi')

all_nums <- c('global_qol_scale', 'infection_frequency', 'initial_symptom_score', 'persistent_symptom_score', 'pre_pandemic',
                'peri_pandemic', 'peri_infection', 'post_infection', 'promis_cog_score', 'hit6_score', 'fatigue_score',
                'gad7_tot_score', 'phq9_total', 'pss_overall_score', 'social_support_score', 'dvprs_score', 'dyspnoea_12_score',
                'BMI', 'duration', 'psqi_glob', 'mental', 'vs_p_rt_mn_s', 'f_c_rt_mn') 
  
  
all_cats <- c('cpt_profile', 'gender', 'race', 'ethnicity', 
              'racial_stress', 'nicotine_use', 'rehab', 'unemployment_covid', 'financial_stress_1', 'func_status_1', 
              'pcfs_grade_status', 'meds', 'tbi_prob', 'tbi_covid', 'vacc_bi', 'cardio_bi')



trim_nums <- c('global_qol_scale', 'infection_frequency', 'initial_symptom_score', 'pre_pandemic',
                'post_infection', 'promis_cog_score', 'hit6_score', 'fatigue_score',
                'gad7_tot_score', 'phq9_total', 'pss_overall_score', 'social_support_score', 'dvprs_score', 'dyspnoea_12_score',
                'BMI', 'duration', 'psqi_glob', 'mental', 'vs_p_rt_mn_s', 'f_c_rt_mn') 
               
trim_cats <- c('cpt_profile', 'gender', 'race', 'ethnicity', 
              'racial_stress', 'nicotine_use', 'rehab', 'unemployment_covid', 'financial_stress_1', 'func_status_1', 
              'pcfs_grade_status', 'meds', 'tbi_prob', 'tbi_covid', 'vacc_bi', 'cardio_bi')



trim_all <- c('global_qol_scale', 'initial_symptom_score', 'pre_pandemic',
              'promis_cog_score', 'hit6_score', 'fatigue_score', 'participant_type',
              'gad7_tot_score', 'phq9_total', 'pss_overall_score', 'social_support_score', 'dvprs_score', 'dyspnoea_12_score',
              'BMI', 'duration', 'psqi_glob', 'mental', 'vs_p_rt_mn_s', 'f_c_rt_mn','cpt_profile', 'gender', 'race', 'ethnicity',
              'racial_stress', 'nicotine_status', 'rehab_status', 'unemployment_covid', 'fin_stress', 'func_status_1', 
              'pcfs_grade_status', 'meds', 'tbi_prob', 'vacc_bi', 'cardio_bi', 'vaccine_menstrual',
              'med_access', 'cluster', 'pcs_diag')

cog_all <- c('promis_cog_score', 'pcs_diag', 'vs_a_rt_mn_s', 'vs_a_rt_sd_s', 'vs_a_acc_s', 
...
all <- c('global_qol_scale', 'initial_symptom_score', 'pre_pandemic',
              'promis_cog_score', 'hit6_score', 'fatigue_score', 'participant_type',
              'gad7_tot_score', 'phq9_total', 'pss_overall_score', 'social_support_score', 'dvprs_score', 'dyspnoea_12_score',
              'BMI', 'duration', 'psqi_glob', 'mental', 'vs_p_rt_mn_s', 'f_c_rt_mn','cpt_profile', 'gender', 'race', 'ethnicity',
              'racial_stress', 'nicotine_status', 'rehab_status', 'unemployment_covid', 'fin_stress', 'func_status_1', 
              'pcfs_grade_status', 'meds', 'tbi_prob', 'vacc_bi', 'cardio_bi', 'vaccine_menstrual',
              'med_access', 'pcs_diag', 'promis_cog_score', 'cluster', 'pcs_diag', 'vs_a_rt_mn_s', 'vs_a_rt_sd_s', 'vs_a_acc_s', 
             'vs_p_rt_mn_s', 'vs_p_rt_sd_s', 'vs_p_acc_s',
             'vs_a_rt_mn_l', 'vs_a_rt_sd_l', 'vs_a_acc_l',
             'vs_p_rt_mn_l', 'vs_p_rt_sd_l', 'vs_p_acc_l',
             'f_c_rt_mn', 'f_c_rt_sd', 'f_c_acc',
...
match_data1 <- samp_match_data[ , (names(samp_match_data) %in% all)]
...
all_data1 <- clean[ , (names(clean) %in% all)]
...
all_data <- write_csv(all_data1, 'all_data.csv')
#dropmatch_data <- write_csv(dropmatch_data1, 'drop_data.csv')
```
